...
export const useAdminsTemplates = defineStore({
  ...
  actions: {
    ...
    async getTemplatesData() {
      try {
        const response = await axios.get(`/api/pwa-templates`);

        this.admins_templates = response.data.data;
      } catch (error) {
        console.error(error);
      }
    },
    ...
    async createTemplate(body: CreateTemplate) {
      try {
        await axios.post('/api/pwa-templates', body);
    ...
    async getDataCreateTemplate(id: number) {
      try {
        const data = await axios.get(`/api/pwa-templates/${id}/edit`);
        return data.data;
      } catch (err) {
        console.log(err);
      }
    ...
    async updateTemplate(id: number, body: CreateAdminsTemplate) {
      try {
        await axios.put(`/api/pwa-templates/${id}`, body);
